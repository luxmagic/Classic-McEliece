...
static void ReloadDirectoryFiles(GuiWindowFileDialogState *state)
{
    ...
    for (int i = 0; i < state->dirFiles.count; i++)
    {
        if (IsPathFile(state->dirFiles.paths[i]))
        {
            ...
            else if (IsFileExtension(state->dirFiles.paths[i], ".txt;.info;.md;.nfo;.xml;.json;.c;.cpp;.cs;.lua;.py;.glsl;.vs;.fs;.pem;.crt"))
            {
...
static bool FileExtension(const char *fileName, const char *ext)
{
    bool result = false;
    const char *fileExt;
